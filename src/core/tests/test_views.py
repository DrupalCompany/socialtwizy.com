...
class TestViews(TestCase):
    ...
    def test_get_home_response(self):
        ...
        self.assertIn('<Page 1 of 1>', str(response.context['groups']))

    def test_get_home_response_no_logged(self):
        response = self.other_client.get(reverse('groups'))
        self.assertRedirects(response, '/?next=/groups/', status_code=302)
